...
#if defined(NDEBUG)
# error "crimson cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "crimson Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s CRM additional fees, %d delta bytes\n",
...
CAmount GetCRMSubsidy(int nHeight, const Consensus::Params& consensusParams) {
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
	...
	if(nHeight < consensusParams.nDiffChangeTarget) {
		...
		nSubsidy = GetCRMSubsidy(nHeight, consensusParams);
	}

	//make sure the reward is at least 1 CRM
	if(nSubsidy < COIN) {
		nSubsidy = COIN;
	}
