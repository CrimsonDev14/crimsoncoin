...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total crimson balance of the wallet\n"
    ...
    UniValue operator()(const CScriptID &scriptID) const {
        ...
        if (pwalletMain && pwalletMain->GetCScript(scriptID, subscript)) {
            ...
            BOOST_FOREACH(const CTxDestination& addr, addresses)
                a.push_back(CcrimsonAddress(addr).ToString());
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nReturn information about the given crimson address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The crimson address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The crimson address validated\n"
    ...
    CcrimsonAddress address(request.params[0].get_str());
...
#ifdef ENABLE_WALLET
        // Case 1: crimson address and we have full public key:
        CcrimsonAddress address(ks);
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are crimson addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) crimson address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
    ...
    CcrimsonAddress address(innerID);
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The crimson address to use for the signature.\n"
    ...
    CcrimsonAddress addr(strAddress);
    if (!addr.IsValid())
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw runtime_error(
            ...
            "crimson-cli and the GUI. There is no server-side difference."
        );
