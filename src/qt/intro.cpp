    ...
    signalled(false)
{
    ...
    if (pruneTarget) {
        uint64_t pruneCRMs = std::ceil(pruneTarget * 1024 * 1024.0 / GB_BYTES);
        if (pruneCRMs <= requiredSpace) {
            requiredSpace = pruneCRMs;
        }
    }
...
bool Intro::pickDataDirectory()
{
    ...
    if(!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) || GetBoolArg("-choosedatadir", DEFAULT_CHOOSE_DATADIR) || settings.value("fReset", false).toBool() || GetBoolArg("-resetguisettings", false))
    {
        ...
        intro.setWindowIcon(QIcon(":icons/crimson"));
     ...
     * override -datadir in the crimson.conf file in the default data directory
     * (to be consistent with crimsond behavior)
     */
    if(dataDir != getDefaultDataDirectory())
