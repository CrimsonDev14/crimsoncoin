...
The RPM spec file provided here is for crimson-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://crimson.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[crimson git repository](https://github.com/crimson/crimson) but are not part of
...
Currently only `Source10` is used, for the example `crimson.conf` file.
...
in the crimson git repository. At present only an SVG version of the crimson
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the crimson
...
needed when 0.12.1 is released, a proper fix is already in the crimson git
master branch.
...
idea to build crimson Core against a static version of that library compiled
...
The `crimson.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `crimson-server` package. At this point, most RPM based
...
The `crimson-server` package should have SELinux support. How to properly do
...
crimson-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/crimson/). He has been
packaging crimson for Fedora at least since 2012.
...
4. I renamed the `crimson` package that contains the Qt GUI to `crimson-core` as
that appears to be how the general population refers to it, in contrast to
`crimson-xt` or `crimson-classic`. I wanted to make sure the general population
