#!/bin/bash
# Copyright (c) 2013 The crimson Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete crimson database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no crimson datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old crimson datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected crimson 0.7 datadir."
    ;;
  3)
    echo "Detected crimson pre-0.8 datadir."
    ;;
  4)
    echo "Detected crimson 0.8 datadir."
    ;;
esac
